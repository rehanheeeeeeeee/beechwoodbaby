...
export default function ViewOrders({ orders }) {
  ...
  const [selectedOrderId, setSelectedId] = useState("");
  const handleClick = (event) => {
    if (event.currentTarget.dataset.field === "orderdetails") {
      ...
      const rowIndex = event.currentTarget.parentElement.data.rowIndex;
      console.log(
        `A cell from the delivery status column just got clicked and it belong to the row at index ${rowIndex}`
      );
    }
  };
