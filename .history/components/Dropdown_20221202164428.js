  ...
  return (
    <div className="text-right">
      <Menu as="div" className="relative inline-block text-left">
        <div>
          <Menu.Button
            className={`w-full justify-center space-x-2 rounded-lg text-[17px] font-medium ${
              style
                ? "text-slate-50 shadow-md hover:shadoe-lg transtion-all ease-in duration-200 p-2 bg-headingColor"
                : "text-textColor"
            }  flex items-center`}
          >
            {({ active }) => (
              <>
                <p>Our Products</p>
                <BiChevronDown
                  className={`${active ? "rotate-180" : "rotate-0"} text-2xl`}
                />
              </>
            )}
          </Menu.Button>
