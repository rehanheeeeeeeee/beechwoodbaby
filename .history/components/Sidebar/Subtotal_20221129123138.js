...
export default function Subtotal() {
  ...
  return (
    <div className="bg-cartTotal rounded-t-[28px] px-8 space-y-3">
      {amounts.map((amount, index) => (
        <Amount key={index} title={amount.title} amount={amount.amount} />
      ))}
      <div className="w-full h-[1px] bg-slate-50 my-2"></div>
      <div className="flex w-full items-center justify-between text-slate-50 text-2xl font-bold">
        <p>Total</p>
        <p>{basketTotal + 0.5}</p>
      </div>
    </div>
